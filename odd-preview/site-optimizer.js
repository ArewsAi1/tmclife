(function(){
  ...
  function normalizeTopNavigation(){
    ...
    document.querySelectorAll('.nav > ul > li > a,.wsite-menu-default > li > a').forEach(function(link){
      ...
      if(routes[key]){link.setAttribute('href',routes[key]);link.onclick=null;link.removeAttribute('data-membership-required');}
    });
  }
  function pruneLegacyNavigation(){
    var links=document.querySelectorAll('.nav a,.wsite-menu a');
    links.forEach(function(link){var path=normalizePath(link.getAttribute('href')||'');if(path==='blog.html'||path==='blog'){var item=link.closest('li');if(item){var submenu=item.querySelector(':scope > .wsite-menu-wrap');if(submenu)submenu.remove();item.classList.remove('wsite-menu-item-wrap-has-children');link.removeAttribute('aria-haspopup');}}});
    document.querySelectorAll('.wsite-menu').forEach(function(menu){var seen=new Set();Array.from(menu.children).forEach(function(item){var link=item.querySelector(':scope > a');if(!link)return;var key=(normalizePath(link.getAttribute('href')||'')+'|'+(link.textContent||'').trim().toLowerCase());if(key!=='|'&&seen.has(key))item.remove();else seen.add(key);});});
    document.querySelectorAll('.nav > ul > li').forEach(function(topItem){var topLink=topItem.querySelector(':scope > a');if(!topLink)return;var text=(topLink.textContent||'').trim().toLowerCase();var path=normalizePath(topLink.getAttribute('href')||'');if(text.indexOf('our products')===-1&&path!=='products-ogden-deck-depot.html')return;topItem.querySelectorAll('.wsite-menu .wsite-menu .wsite-menu-wrap').forEach(function(deepMenu){deepMenu.remove();});topItem.querySelectorAll('.wsite-menu .wsite-menu-item-wrap-has-children').forEach(function(item){item.classList.remove('wsite-menu-item-wrap-has-children');});});
  }
  function improveControls(){
    document.querySelectorAll('a[target="_blank"]').forEach(function(link){var rel=new Set((link.getAttribute('rel')||'').split(/\s+/).filter(Boolean));rel.add('noopener');rel.add('noreferrer');link.setAttribute('rel',Array.from(rel).join(' '));});
  ...
  function restoreContactPage(){
    var currentPath=normalizePath(location.href);if(currentPath!=='contact.html'&&currentPath!=='contact')return;
    var content=document.querySelector('#wsite-content,.main-wrap .container,.wsite-section-elements');if(!content)return;
    Array.from(content.querySelectorAll('h1,h2,h3,div.paragraph,p')).forEach(function(el){if((el.textContent||'').trim().toLowerCase()==='under construction')el.textContent='Contact Ogden Deck Depot';});
    if(document.getElementById('odd-contact-form'))return;
    var section=document.createElement('section');section.id='odd-contact-form';section.className='odd-contact-card';
    ...
    content.appendChild(section);var form=section.querySelector('form');form.addEventListener('submit',async function(event){event.preventDefault();var status=section.querySelector('.odd-form-status');var button=form.querySelector('button');status.textContent='Sending...';button.disabled=true;try{var response=await fetch('/api/lead',{method:'POST',body:new FormData(form)});var result=await response.json();if(!response.ok||!result.ok)throw new Error(result.error||'Unable to send request.');status.textContent=result.message||'Thanks — your request was received.';form.reset();}catch(error){status.textContent=(error&&error.message)||'Please call (435) 222-5819.';}finally{button.disabled=false;}});
  }
